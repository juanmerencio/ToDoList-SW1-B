...
export default async function tasksRoutes(app) {
    ...
    app.get('/api/tasks', async (request, reply) => {
        ...
        const [[totals]] = await pool.query('SELECT COUNT(*) AS total, SUM(completed = 1) AS completedCount FROM tasks');
        return reply.send({
            tasks: rows,
            total: totals.total,
    ...
    app.post('/api/tasks', async (request, reply) => {
        ...
        const [result] = await pool.query('INSERT INTO tasks (title, completed, due_date) VALUES (?, false, ?)',
